...
class DenseBiEncoder(nn.Module):
    ...
    def encode(self, input_ids, attention_mask, grading=False, **kwargs):
        ...
        **kwargs: 
        ...
        """
        # BEGIN SOLUTION
        # NOTE: This does not exclude the padding tokens.
        last_hidden_state = self.model(input_ids, attention_mask=attention_mask, **kwargs).last_hidden_state
        expanded_attention_mask = attention_mask.unsqueeze(-1).expand_as(last_hidden_state).float()
        masked_hidden_state = last_hidden_state * expanded_attention_mask
        summed_embeddings = masked_hidden_state.sum(dim=1)
        num_tokens = expanded_attention_mask.sum(dim=1)
        reps = summed_embeddings / num_tokens.clamp(min=1)
        # END SOLUTION
        return {'reps':reps, 'last_hidden_state':last_hidden_state} if grading else reps
    ...
    def score_pairs(self, queries, docs):
        ...
        return torch.einsum('ij,ij->i', queries, docs)
        # END SOLUTION
    ...
    def save_pretrained(self, model_dir, state_dict=None):
        ...
        """
        self.model.save_pretrained(model_dir, state_dict=state_dict, safe_serialization=False)
