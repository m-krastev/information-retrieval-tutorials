...
def logit_to_prob(logit):
    ### BEGIN SOLUTION
    # Compute the probability from the logit
...
def train_unbiased_listNet(net, params, data):
    ...
    for epoch in range(params.epochs):
        ...
        for (features, clicks, positions) in train_loader:
            ...
            output = net(features)
            # Compute the loss
            loss = unbiased_listNet_loss(output, clicks, params.propensity[positions])
            # Backward pass
            loss.backward()
            # Optimize
            optimizer.step()
...
def train_DLA_listNet(net, params, data):
    ...
    for epoch in range(params.epochs):
        ...
        for (features, clicks, positions) in train_loader:
            ...
            # Compute the loss
            loss_relevance = unbiased_listNet_loss(output, clicks, logit_to_prob(prop_output.squeeze()))
            loss_propensity = unbiased_listNet_loss(prop_output, clicks, logit_to_prob(output.squeeze()))
