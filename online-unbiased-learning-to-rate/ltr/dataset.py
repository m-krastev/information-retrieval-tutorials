...
class ClickLTRData(Dataset):
    ...
    def __getitem__(self, q_i):
        ...
        select = positions < self.logging_policy.topk
        tensor_features = torch.from_numpy(self.split.query_feat(q_i)[select]).float()
        tensor_clicks = torch.from_numpy(clicks[select]).float()
        tensor_positions = torch.from_numpy(positions[select])
        ### END SOLUTION
