...
def convert_to_float(obj):
    if isinstance(obj, tuple):
        return tuple(float(x) if isinstance(x, np.float32) else x for x in obj)
    elif isinstance(obj, list):
        return [convert_to_float(item) for item in obj]
    elif isinstance(obj, dict):
        return {k: convert_to_float(v) for k, v in obj.items()}
    else:
        return float(obj) if isinstance(obj, np.float32) else obj
...
def create_results(data, net, train_fn, prediction_fn, results_file, *train_params):
    ...
    test_metrics = convert_to_float(test_metrics)
    test_q = convert_to_float(test_q)
